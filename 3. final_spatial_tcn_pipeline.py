...
# ================================================================
# ENHANCED SPATIAL TCN ARCHITECTURE
# ================================================================
...
class ProductionSpatialTCN(nn.Module):
    ...
    def forward(self, x):
        ...
        # Classifier with residual-like connections
        identity = x
...
class SpatialTCNTrainer:
    def __init__(self, model, device, save_dir='production_models'):
        ...
        for epoch in range(epochs):
            ...
            if val_acc > self.best_val_acc:
                ...
                if save_best:
                    ...
                    self.best_model_path = self.save_dir / f"best_spatial_tcn_{timestamp}_{val_acc:.4f}.pth"
...
def create_production_pipeline(retrain_from_scratch=True, 
                              existing_model_path='production_models/best_spatial_tcn_20250616_235932_0.7328.pth',
                              augmentation_strategy='savgol_minmax'):
    """
    Complete production pipeline for Spatial TCN
    ...
    print("🏭 SPATIAL TCN PRODUCTION PIPELINE")
    print("="*50)
    ...
    # Save preprocessor
    preprocessor.save('production_models/preprocessor.pkl')
    ...
    # Setup trainer
    trainer = SpatialTCNTrainer(model, device)
    ...
    if not retrain_from_scratch and existing_model_path:
        ...
        # for param in model.tcn1.parameters():
        #     param.requires_grad = False
    ...
    history = trainer.train(
        ...
        epochs=epochs,
        early_stopping_patience=20,  # Increased from 10 to 20
        save_best=True,
        verbose=True
    )
    ...
    evaluator.plot_confusion_matrix(
        ...
        save_path='production_models/confusion_matrix.png'
    )
    ...
    with open('production_models/final_results.json', 'w') as f:
        json.dump(final_results, f, indent=2, default=str)
    
    print(f"\n✅ Pipeline completed!")
    print(f"   Model saved: {trainer.best_model_path}")
    print(f"   Results saved: production_models/final_results.json")
    print(f"   Preprocessor saved: production_models/preprocessor.pkl")
...
if __name__ == "__main__":
    # Run the complete pipeline with transfer learning
    pipeline_results = create_production_pipeline(
        retrain_from_scratch=False,  # Changed to False to use transfer learning
        existing_model_path='production_models/best_spatial_tcn_20250616_235932_0.7328.pth',
        augmentation_strategy='savgol_minmax'
    ) 
...
class TCNTrainer:
    def __init__(self, model, device, save_dir='final_tcn_models'):
        ...
        for epoch in range(epochs):
            ...
            if val_acc > self.best_val_acc:
                ...
                if save_best:
                    ...
                    self.best_model_path = self.save_dir / f"best_tcn_{timestamp}_{val_acc:.4f}.pth"
...
def create_tcn_pipeline(retrain_from_scratch=True, 
                       existing_model_path=None,
                       augmentation_strategy='savgol_minmax'):
    """
    Complete production pipeline for basic TCN
    ...
    print("🏭 BASIC TCN PRODUCTION PIPELINE")
    print("="*50)
    ...
    # Save preprocessor
    preprocessor.save('final_tcn_models/preprocessor.pkl')
    ...
    print(f"\n🏗️  Creating Basic TCN model...")
    model = ProductionTCN(
        in_channels=7,
        num_classes=3,
        num_filters=32,    # As specified
        kernel_size=3,     # As specified
        dropout=0.3        # As specified
    )
    
    print(f"   Parameters: {sum(p.numel() for p in model.parameters()):,}")
    print(f"   Hyperparameters: num_filters=32, kernel_size=3, dropout=0.3")
    ...
    evaluator.plot_confusion_matrix(
        ...
        save_path='final_tcn_models/confusion_matrix.png'
    )
    ...
    with open('final_tcn_models/final_results.json', 'w') as f:
        json.dump(final_results, f, indent=2, default=str)
    
    print(f"\n✅ Pipeline completed!")
    print(f"   Model saved: {trainer.best_model_path}")
    print(f"   Results saved: final_tcn_models/final_results.json")
    print(f"   Preprocessor saved: final_tcn_models/preprocessor.pkl")
...
if __name__ == "__main__":
    # Run the complete pipeline with specified hyperparameters
    pipeline_results = create_tcn_pipeline(
